...
interface ImageEditorProps {
    onSendToListing?: (imageDataUrl: string) => void
}

export default function ImageEditor({ onSendToListing }: ImageEditorProps) {
    ...
    // Send canvas image to listing
    const sendToListing = () => {
        const canvas = canvasRef.current
        if (!canvas || !onSendToListing) return
                                ...
                                <div className="flex gap-2">
                                    {onSendToListing && (
                                        <Button
                                            ...
                                            onClick={sendToListing}
                                        >
                                            <Send className="w-3.5 h-3.5 mr-2" /> Send to Listing
                                        </Button>
                                    )}
                                    <Button
                                        size="sm"
                            ...
                            {elements.length === 0 ? (
                                ...
                                <div className="space-y-1">
                                    {[...elements].reverse().map((el, idx) => (
                                        <div key={el.id}>
                                            <div
                                                className={`flex items-center gap-2 p-2 cursor-pointer transition-colors ${el.id === selectedElementId && !selectedSlotId
                                            ...
                                            {el.type === 'collage' && el.id === selectedElementId && (
                                                <div className="ml-4 space-y-0.5 mt-0.5">
                                                    {(el as CollageElement).slots.map((slot, slotIdx) => (
                                                        <div
                                                            key={slot.id}
                                                            className={`flex items-center gap-2 p-1.5 cursor-pointer transition-colors ${slot.id === selectedSlotId
